 ...
 * WARN This query fails if both count and cursor are set
 */
const getNameRecordFromAntQuery = (includeCount = false) => gql`
  query (
    $antId: String!
    $limit: Int!
    $sortOrder: SortOrder!
    ...
    transactions(
      ...
      tags: [${AO_NETWORK_IDENTIFIER}, { name: "Action", values: ["Buy-Record"] }, { name: "Process-Id", values: [$antId]}]
      recipients: ["${import.meta.env.VITE_ARNS_AR_IO_REGISTRY}"]
...
export async function getNameRecordFromAnt(
  limit = 100,
  cursor = "",
  ascending: boolean,
  //
  ...
  try {
    const result = await goldsky
      .query<TransactionsResponse>(getNameRecordFromAntQuery(!cursor), {
        limit,
        sortOrder: ascending ? "INGESTED_AT_ASC" : "INGESTED_AT_DESC",
        cursor,
        //
